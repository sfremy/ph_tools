...
def run_starpasta(masses, metallicities, ML_on=True, verbose=False, stop_LM=True, fast_SN=True):
    '''
    Run Star Pasta stellar evolution code on lists of masses and metallicities.
    
    Parameters:
    Basic Arguments:
        masses (list of float): Initial masses of the stars in solar masses.
        metallicities (list of float): Metallicity values (Z) for the stars.

    Options:
        ML_on (bool): Toggle mass loss and small envelope adjustment. Default is True.
        verbose (bool): If True, the script reports each timestep in the command window. Default is False.
        stop_LM (bool): If True, stops stars <0.7 solar masses from continuing past the main sequence. Default is True.
        fast_SN (bool): If True, uses remnant mass formulae from Fryer et al. 2012. Default is True.
    '''

    for M0, Z in zip(masses, metallicities):


    path = os.path.join(os.path.dirname(__file__), '') #CHANGE THIS
